...
const mGun = mm(0x181818, 0.3, 0.85),
  mGun2 = mm(0x221810, 0.6, 0.2),
  mGunW = mm(0x2a1a0a, 0.5, 0.05);
export const wpn = new THREE.Group();
camera.add(wpn);
const aw = (g, mat, x, y, z) => {
...
const brl = new THREE.Mesh(new THREE.CylinderGeometry(0.009, 0.009, 0.36, 8), mGun);
brl.rotation.x = Math.PI / 2;
brl.position.set(0, 0.012, -0.33);
wpn.add(brl);
...
const mzDv = new THREE.Mesh(new THREE.CylinderGeometry(0.013, 0.013, 0.04, 8), mGun);
mzDv.rotation.x = Math.PI / 2;
mzDv.position.set(0, 0.012, -0.51);
wpn.add(mzDv);
wpn.position.set(0.11, -0.105, -0.2);
export const flashMat = new THREE.MeshBasicMaterial({
  color: 0xffdd55,
  transparent: true,
  opacity: 0,
});
