...
const fill=new THREE.DirectionalLight(0x6080a0,0.4*Math.PI);fill.position.set(-30,20,-20);scene.add(fill);
const torchLights=[];
[[3,4],[11,3],[20,4],[5,11],[18,11],[3,19],[11,20],[20,20],[11,12]].forEach(([tc,tr])=>{
  const tl=new THREE.PointLight(0xff8822,1.8*4*Math.PI,12);tl.position.set(tc*CELL,2.2,tr*CELL);
  tl.userData.base=1.8*4*Math.PI;tl.userData.ft=Math.random()*100;scene.add(tl);torchLights.push(tl);
});
