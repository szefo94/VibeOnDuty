...
(function buildLevel() {
  ...
  const fl = new THREE.Mesh(
    new THREE.PlaneGeometry((MAP_W + 4) * CELL, (MAP_H + 4) * CELL),
    matFloor
  );
  fl.rotation.x = -Math.PI / 2;
  fl.position.set((MAP_W * CELL) / 2, 0, (MAP_H * CELL) / 2);
  ...
  for (let row = 0; row < MAP_H; row++) {
    for (let col = 0; col < MAP_W; col++) {
      ...
      const wx = col * CELL + CELL / 2,
        wz = row * CELL + CELL / 2;
      if (cell === 1) {
        const n = mapCell(col, row - 1),
          s = mapCell(col, row + 1),
          e2 = mapCell(col + 1, row),
          w2 = mapCell(col - 1, row);
        const isPillar = n === 0 && s === 0 && e2 === 0 && w2 === 0;
        if (isPillar) {
          const shaft = new THREE.Mesh(
            new THREE.CylinderGeometry(0.55, 0.62, WALL_H + 0.8, 12),
            matWall
          );
          shaft.position.set(wx, (WALL_H + 0.8) / 2, wz);
          shaft.castShadow = shaft.receiveShadow = true;
          scene.add(shaft);
          wallMeshes.push(shaft);
          const cap = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.35, 1.4), matWallT);
          cap.position.set(wx, WALL_H + 0.8, wz);
          scene.add(cap);
          const base = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.22, 1.5), matWallT);
          ...
          const w = new THREE.Mesh(new THREE.BoxGeometry(CELL, WALL_H, CELL), [
            matWallD,
            matWallD,
            matWallT,
            matFloor,
          ...
          scene.add(w);
          wallMeshes.push(w);
          const t = new THREE.Mesh(new THREE.BoxGeometry(CELL, 0.18, CELL), matTrim);
        ...
        const loH = PLAYER_H - 0.28,
          upH = 0.55;
        const gw = cell === 2 ? CELL : 0.35,
          gd = cell === 2 ? 0.35 : CELL;
        ...
        const rampMat = new THREE.MeshStandardMaterial({
          color: matRamp.color,
          roughness: matRamp.roughness,
          metalness: matRamp.metalness,
  ...
  for (let i = 0; i < 80; i++) {
    const rc = 1 + Math.floor(Math.random() * (MAP_W - 2)),
      rr = 1 + Math.floor(Math.random() * (MAP_H - 2));
    ...
    rb.position.set(
      rc * CELL + Math.random() * CELL * 0.8,
      sz * 0.25,
      rr * CELL + Math.random() * CELL * 0.8
    );
  ...
  const elevMat = mm(0xb8a070, 0.94, 0.01),
    elevSide = mm(0x8a7050, 0.96, 0.01);
  const built = new Set();
  for (let r = 0; r < MAP_H; r++) {
    for (let c = 0; c < MAP_W; c++) {
      const h = HMAP[r][c];
      if (!h || MAP[r][c] !== 0) continue;
      const key = `${c},${r}`;
      ...
      while (
        ...
        w++;
      let d = 1;
      outer: while (r + d < MAP_H) {
        for (let cc = c; cc < c + w; cc++) {
          if (HMAP[r + d][cc] !== h || MAP[r + d][cc] !== 0 || built.has(`${cc},${r + d}`)) {
            break outer;
          }
        }
        d++;
      ...
      for (let dr = 0; dr < d; dr++)
        for (let dc = 0; dc < w; dc++) built.add(`${c + dc},${r + dr}`);
      const pw = w * CELL,
        pd = d * CELL,
        cx2 = (c + w / 2) * CELL,
        cz2 = (r + d / 2) * CELL;
      const slab = new THREE.Mesh(new THREE.BoxGeometry(pw, h, pd), [
        elevSide,
        elevSide,
        elevMat,
        elevMat,
...
function buildDebugLines() {
  ...
  for (const d of debugLineData) {
    ...
    [
      ...
      [2, 6],
      [3, 7],
    ].forEach(([a, b]) => groups[k].pts.push(...corners[a], ...corners[b]));
  }
